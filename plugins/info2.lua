...
local function res_user_callback(extra, success, result) -- /info <username> function
  ...
  if result.username then
   ...
   Username = 'ندارد'
    ...
    if not value then
	 if result.id == tonumber(💎 ‌KING OF KINGS 👑) then
  ...
  local text = 'نام : '..(result.from.first_name or '')..'\nفامیل :'..(result.from.last_name or '----')..'\n'
	...
	local hash = 'مقام:'..result.to.id..':variables'
		local value = redis:hget(hash, result.from.id)
		 if not value then
		    if result.from.peer_id == tonumber(💎 ‌KING OF KINGS 👑) then
		       text = text..'مقام : مدیر کل ربات Executive Admin \n\n'
  ...
  if msg.from.username then
   ...
   local hash = 'rank:variables'
	if hash then
	  ...
	  if not value then
		if msg.from.id == tonumber(💎 ‌KING OF KINGS 👑) then
		 text = text..'مقام : مدیر کل ربات (Executive Admin) \n\n'
		elseif is_sudo(msg) then
		 text = text..'مقام : ادمین ربات (Admin) \n\n'
		elseif is_owner(msg) theny
		 text = text..'مقام : مدیر کل گروه (Owner) \n\n'
