...
function run(msg, matches)
	if string.match(msg.text, '[\216-\219][\128-\191]') then
		return send_large_msg(get_receiver(msg), '⚫️ فارسی پشتیبانی نمیشود ⚫️\n⚫️ از متن فینگلیش استفاده کنید⚫️ ')
	end
	...
	redis:del(hash)
	if #jdat.response < 2 then return "🔴 هیچ نتیجه ای یافت نشد 🔴" end
		...
		text = text..'⚫️ برای دریافت لینک دانلود از دستور زیر استفاده کنید ⚫️\n/dl <number>\n(example): /dl 1'
	return text
end
